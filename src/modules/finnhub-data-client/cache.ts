...
import { CacheEntry, CacheKey, FinnhubForexPair, FinnhubTimeframe } from './types';

export class FinnhubCache {
  ...
  invalidateSymbol(symbol: FinnhubForexPair): void {
  ...
  invalidateTimeframe(timeframe: FinnhubTimeframe): void {
    const keysToDelete: string[] = [];

    for (const key of this.cache.keys()) {
      const parts = key.split(':');
      if (parts.length >= 2 && parts[1] === timeframe) {
        keysToDelete.push(key);
      }
    }
